...
class ATM(EasyFrame):
    def __init__(self, bank):
        ...
        self.statusField = self.addTextField(row=3, column=1, text="Welcome to the Bank!", state="readonly")

        self.balanceButton = self.addButton(row=0, column=2, text="Balance", command=self.getBalance, state="disabled")
        self.depositButton = self.addButton(row=1, column=2, text="Deposit", command=self.deposit, state="disabled")
        self.withdrawButton = self.addButton(row=2, column=2, text="Withdraw", command=self.withdraw, state="disabled")
        self.loginButton = self.addButton(row=3, column=2, text="Login", command=self.login)
    ...
    def login(self):
        ...
        if self.account:
            ...
            if self.failedAttempts >= 3:
                self.popUp("Alert", "3 failed attempts! The police will be called!")
    ...
    def getBalance(self):
        self.statusField.setText(f"Balance = ${self.account.getBalance()}")
